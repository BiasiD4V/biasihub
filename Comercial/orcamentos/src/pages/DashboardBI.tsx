...
import {
  Filter, X, TrendingUp, DollarSign, Target,
  FileText, Users,
  BarChart3, Calendar, Building2, Briefcase, RefreshCw,
} from 'lucide-react';
import { supabase } from '../infrastructure/supabase/client';
import {
  type PropostaRow, type Filtros,
  STATUS_CORES, CORES_GRAFICO,
  FMT_BRL, FMT_BRL_SHORT,
  CustomTooltip, FilterChip, KPI, Painel,
...
export function DashboardBI() {
  ...
  const carregarDados = useCallback(async () => {
    ...
    if (propostasRes.error) setErro('Falha ao carregar dados. Verifique sua conexão com o banco.');
    else if (propostasRes.data) { setRows(propostasRes.data); setErro(null); }
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      {erro && (
        <div className="mx-4 lg:mx-8 mt-4 flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-700">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          <span className="font-medium">{erro}</span>
          <button onClick={carregarDados} className="ml-auto text-xs font-bold underline hover:no-underline">Tentar novamente</button>
        </div>
      )}
      {/* ── Header ── */}
