...
import { Send, Bot, User, Sparkles, RefreshCw, Package, ClipboardList, HardHat, Truck } from 'lucide-react';
...
const SUGESTOES = [
  ...
  { icone: HardHat, texto: 'Algum EPI vencendo em breve?' },
  { icone: Truck, texto: 'Como está a frota hoje?' },
];

const SAUDACOES_PAULO = [
  'Oi! Acabei de estudar os dados do almoxarifado. Me pergunta qualquer coisa!',
  'Olá! Estive analisando aqui e tenho bastante coisa pra te contar. O que quer saber?',
  'Ei! Sempre estudando aqui. Me perguntar o que precisar sobre estoque, frota, EPI...',
];
...
export function Paulo() {
  ...
  useEffect(() => {
    async function carregarHistorico() {
      ...
      if (data && data.length > 0) {
        ...
        // Primeira vez — Paulo se apresenta
        const saudacao = SAUDACOES_PAULO[Math.floor(Math.random() * SAUDACOES_PAULO.length)];
  ...
  async function enviar(texto?: string) {
    ...
    try {
      ...
      const msgPaulo: Mensagem = { papel: 'assistant', conteudo: resposta };
      setMensagens(prev => [...prev, msgPaulo]);

      // Salva resposta do Paulo
      if (usuario?.id) {
        supabase.from('conversas_paulo').insert({
          usuario_id: usuario.id,
          papel: 'assistant',
    ...
    <div className="flex flex-col h-full max-h-[calc(100vh-0px)]">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-slate-800 leading-none">Paulo</h1>
      ...
      <div className="px-4 py-3 bg-white border-t border-slate-100 flex-shrink-0">
        <div className="flex gap-2 items-center bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
          <input
            ...
            placeholder="Pergunte qualquer coisa para o Paulo..."
        ...
        <p className="text-center text-[10px] text-slate-400 mt-2">
          Paulo usa dados reais do almoxarifado para te ajudar
        </p>
      </div>
    </div>
  );
