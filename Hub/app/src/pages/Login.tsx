...
export function Login() {
  ...
  function handleSolicitarAcesso(e: FormEvent) {
    e.preventDefault();
    const subject = encodeURIComponent('Solicitação de acesso — BiasíHub');
    const body = encodeURIComponent(
      `Olá,\n\nO usuário abaixo está solicitando acesso ao BiasíHub:\n\nNome: ${nomeNovo}\nE-mail: ${emailNovo}\n\nPor favor, crie a conta e envie as credenciais.\n\nAtt.`
    );
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
          <div className="mb-6 text-center">
            <img src="/logo-biasi.png" alt="Biasi Engenharia" className="h-12 w-auto mx-auto mb-4" />
            <h1 className="text-2xl font-bold text-slate-800">Entrar</h1>
            <p className="text-sm text-slate-500 mt-1">BiasíHub — Portal Central</p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-lg w-full max-w-sm p-8 relative">
            ...
            {solicitacaoEnviada ? (
              <div className="text-center py-4">
                ...
                <button onClick={() => setModalCriar(false)} className="mt-6 w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 rounded-lg text-sm transition-colors">
                  Fechar
                </button>
              </div>
            ) : (
              <>
                ...
                <form onSubmit={handleSolicitarAcesso} className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={nomeNovo} onChange={(e) => setNomeNovo(e.target.value)} placeholder="Seu nome completo" required className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">E-mail</label>
                    <input type="email" value={emailNovo} onChange={(e) => setEmailNovo(e.target.value)} placeholder="seu@email.com.br" required className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow" />
                  </div>
