...
type InsumoOpt = { id: string; codigo: string; descricao: string; unidade: string | null; grupo: string | null };
type FerramentaOpt = { id: string; codigo: string; descricao: string; unidade: string | null; marca: string | null; categoria: string | null; estoque_atual: number | null };
type VeiculoOpt = { id: string; placa: string | null; modelo: string; marca: string | null };
type ItemLinha = { uid: string; itemId: string | null; codigo: string | null; descricao: string; quantidade: string; unidade: string; observacao: string; foto?: File | null; fotoUrl?: string; marca?: string | null; categoria?: string | null; grupo?: string | null; placa?: string | null; modelo?: string | null };
...
function Autocomplete<T extends AutocompleteItem>({ items, placeholder, loading, valueId, onSelect }: { items: T[]; placeholder: string; loading: boolean; valueId: string | null; onSelect: (item: T | null) => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((i) => i.titulo.toLowerCase().includes(q) || (i.sub || '').toLowerCase().includes(q)).slice(0, 15);
  }, [query, items]);

  return (
    <div ref={wrapRef} className="relative">
      <input type="text" className={styles.inputCompact} placeholder={loading ? 'Carregando...' : placeholder} value={query}
        ...
        <div className="absolute left-0 right-0 top-full mt-1 z-50 rounded-[14px] overflow-hidden border border-[rgba(113,154,255,0.45)] bg-[#0d2258] shadow-xl">
          <ul className="max-h-64 overflow-y-auto">
            {filtered.length === 0 ? (
              ...
              <li key={it.id} className="px-3 py-2.5 text-[0.9rem] text-[#f4f7ff] hover:bg-white/10 cursor-pointer border-b border-white/5 last:border-b-0"
...
function ItemRow({ linha, categoria, insumos, ferramentas, veiculos, loading, onChange, onRemove }: { linha: ItemLinha; categoria: CategoriaType; insumos: InsumoOpt[]; ferramentas: FerramentaOpt[]; veiculos: VeiculoOpt[]; loading: boolean; onChange: (patch: Partial<ItemLinha>) => void; onRemove: () => void }) {
  ...
  const autoItems = useMemo<AutocompleteItem[]>(() => {
    if (categoria === 'insumos') return insumos.map((i) => ({ id: i.id, titulo: `${i.codigo} - ${normalizeDisplayText(i.descricao)}`, sub: `${i.unidade || 'un'}${i.grupo ? ` • ${i.grupo}` : ''}` }));
    if (categoria === 'ferramentas') return ferramentas.map((f) => ({ id: f.id, titulo: `${f.codigo} - ${normalizeDisplayText(f.descricao)}`, sub: `${f.marca || ''}${f.marca ? ' • ' : ''}${f.unidade || 'un'}${f.estoque_atual != null ? ` • estoque ${f.estoque_atual}` : ''}` }));
    return veiculos.map((v) => ({ id: v.id, titulo: `${v.placa ? v.placa + ' - ' : ''}${v.modelo}`, sub: v.marca || '' }));
  ...
  function handleSelect(item: AutocompleteItem | null) {
    ...
    if (categoria === 'insumos') { const raw = insumos.find((i) => i.id === item.id); if (!raw) return; onChange({ itemId: raw.id, codigo: raw.codigo, descricao: normalizeDisplayText(raw.descricao), unidade: raw.unidade || linha.unidade || 'un', grupo: raw.grupo }); }
    else if (categoria === 'ferramentas') { const raw = ferramentas.find((f) => f.id === item.id); if (!raw) return; onChange({ itemId: raw.id, codigo: raw.codigo, descricao: normalizeDisplayText(raw.descricao), unidade: raw.unidade || linha.unidade || 'un', marca: raw.marca, categoria: raw.categoria }); }
    else { const raw = veiculos.find((v) => v.id === item.id); if (!raw) return; onChange({ itemId: raw.id, codigo: raw.placa, descricao: `${raw.placa ? raw.placa + ' - ' : ''}${raw.modelo}`, unidade: 'uso', placa: raw.placa, modelo: raw.modelo, marca: raw.marca }); }
  }
  ...
  function capturePhoto() {
    ...
    canvas.toBlob((blob) => {
      ...
      onChange({ foto: photoFile, fotoUrl: URL.createObjectURL(photoFile) });
      closeCamera();
    }, 'image/jpeg', 0.92);
  }
  ...
  return (
    <div className="p-3.5 rounded-[18px] border border-[rgba(113,154,255,0.28)] bg-[rgba(8,24,64,0.24)]">
      <div className="grid gap-3 items-start" style={{ gridTemplateColumns: 'minmax(0,2.2fr) 120px 180px 52px' }}>
        <div>
          <Autocomplete items={autoItems} placeholder={categoria === 'insumos' ? 'Buscar insumo...' : categoria === 'ferramentas' ? 'Buscar ferramenta...' : 'Buscar veículo...'} loading={loading} valueId={linha.itemId} onSelect={handleSelect} />
        </div>
        <input type="number" min="1" step="any" className={styles.inputCompact} placeholder="Qtd" value={linha.quantidade} onChange={(e) => onChange({ quantidade: e.target.value })} />
        <select className={styles.inputCompact} value={linha.unidade} onChange={(e) => onChange({ unidade: e.target.value })} style={selectFieldStyle}>
          ...
          {linha.unidade && !UNIDADES.includes(linha.unidade) && <option value={linha.unidade} style={selectOptionStyle}>{linha.unidade}</option>}
        </select>
        <button type="button"
      ...
      <div className="mt-3">
        <input type="text" className={styles.inputCompact} placeholder="Observação do item (opcional)" value={linha.observacao} onChange={(e) => onChange({ observacao: e.target.value })} />
      </div>
      <div className="mt-2 flex items-center gap-3">
        <button type="button" className={styles.btnSmall + ' text-[0.8rem] py-1.5'} onClick={openCamera}>
          <Camera size={13} />
          {linha.fotoUrl ? 'Trocar foto' : 'Foto do item'}
        </button>
        <input
          ref={fileInputRef}
          type="file"
          ...
          onChange={(e) => {
            ...
            if (!f) return;
            onChange({ foto: f, fotoUrl: URL.createObjectURL(f) });
          }}
        />
        {linha.fotoUrl && (
          <div className="flex items-center gap-2">
            <img src={linha.fotoUrl} alt="foto" className="w-12 h-12 object-cover rounded-[10px] border border-[rgba(113,154,255,0.4)]" />
            <button type="button" onClick={() => onChange({ foto: null, fotoUrl: '' })} className="text-[#ff9797] text-[0.78rem]">remover</button>
          </div>
        )}
      </div>
      {cameraOpen && (
...
function TipoCard({ active, onClick, pill, icon, titulo, descricao }: { active: boolean; onClick: () => void; pill: string; icon: JSX.Element; titulo: string; descricao: string }) {
  return (
    <button type="button" onClick={onClick}
      className={`text-left relative p-7 min-h-[210px] rounded-[24px] cursor-pointer transition-all border ${active ? 'border-[rgba(92,137,255,0.65)] bg-[linear-gradient(180deg,rgba(25,57,130,0.98),rgba(20,47,110,0.92))] shadow-[0_0_0_2px_rgba(92,137,255,0.35)]' : 'border-[rgba(113,154,255,0.28)] bg-[linear-gradient(180deg,rgba(18,45,109,0.94),rgba(20,47,110,0.84))] hover:-translate-y-[2px] hover:border-[#3560b8]'}`}>
      <div className={styles.pill + ' mb-5 text-white'}>{pill}</div>
      <div className="w-[62px] h-[62px] rounded-2xl grid place-items-center border border-[#3560b8] bg-[rgba(12,29,77,0.35)] mb-4 text-[1.9rem]">{icon}</div>
...
export function RequisicaoPublica() {
  ...
  const [itens, setItens] = useState<ItemLinha[]>([{ uid: uid(), itemId: null, codigo: null, descricao: '', quantidade: '1', unidade: 'un', observacao: '' }]);
  ...
  useEffect(() => {
    async function carregar() {
      ...
      } catch (err) { console.error('[RequisicaoPublica] erro:', err); }
  ...
  function handleSetCategoria(c: CategoriaType) {
    ...
    setCategoria(c);
    setItens([{ uid: uid(), itemId: null, codigo: null, descricao: '', quantidade: '1', unidade: c === 'frota' ? 'uso' : 'un', observacao: '' }]);
  }

  function addLinha() { setItens((prev) => [...prev, { uid: uid(), itemId: null, codigo: null, descricao: '', quantidade: '1', unidade: categoria === 'frota' ? 'uso' : 'un', observacao: '' }]); }
  function updateLinha(u: string, patch: Partial<ItemLinha>) { setItens((prev) => prev.map((l) => (l.uid === u ? { ...l, ...patch } : l))); }
  function removeLinha(u: string) { setItens((prev) => (prev.length > 1 ? prev.filter((l) => l.uid !== u) : prev)); }
  function handleFiles(files: FileList | null) { if (!files) return; setAnexos((prev) => [...prev, ...Array.from(files)]); }
  function removeAnexo(i: number) { setAnexos((prev) => prev.filter((_, idx) => idx !== i)); }
  ...
  async function iniciarGravacao() {
    try {
      ...
      mediaRecorderRef.current = mr; mr.start(); setGravando(true);
  ...
  async function enviar(e?: React.FormEvent) {
    ...
    if (itensValidos.length === 0) return setErro('Selecione ao menos um item com quantidade válida.');
    if (prioridade === 'urgente' && !justificativaUrgencia.trim()) return setErro('Justifique a urgência.');

    const itensSemFoto = itensValidos.filter((l) => !l.foto && !l.fotoUrl);
    if (itensSemFoto.length > 0) return setErro('Foto do item é obrigatória em todos os itens da requisição.');
    if (prazo && new Date(prazo).getTime() < Date.now() - 60_000) return setErro('Prazo desejado não pode ser no passado.');

    setSalvando(true);
    try {
      const tipoJson = categoria === 'insumos' ? 'material' : categoria === 'ferramentas' ? 'ferramenta' : 'carro';
      const itensPayload = itensValidos.map((l) => ({ tipo: tipoJson, item_id: l.itemId, codigo: l.codigo, descricao: l.descricao, nome: l.descricao, quantidade: Number(l.quantidade), unidade: l.unidade, observacao: l.observacao || null, marca: l.marca ?? null, categoria_ferramenta: l.categoria ?? null, grupo: l.grupo ?? null, placa: l.placa ?? null, modelo: l.modelo ?? null, urgente: prioridade === 'urgente' ? 'sim' : 'não', justificativa_urgencia: prioridade === 'urgente' ? justificativaUrgencia : '', choice_estoque: 'ok', foto_material: null, fase_rastreio: 0 }));
      const obsFinal = [prazo ? `prazo:${prazo}` : '', `prioridade:${prioridade}`, observacao ? `obs:${observacao}` : ''].filter(Boolean).join(' | ');
      const tel = telefone.replace(/\\D/g, '');
      localStorage.setItem(IDENT_KEY, JSON.stringify({ nome: nome.trim(), tel }));
      const { error } = await supabase.from('requisicoes_almoxarifado').insert({ solicitante_id: null, solicitante_nome: nome.trim(), telefone: tel, obra: obraFinal, observacao: obsFinal, itens: itensPayload });
      ...
      <div className={styles.page}>
        ...
        <div className="min-h-screen flex items-center justify-center p-4">
          <div className="rounded-[28px] border border-[rgba(113,154,255,0.28)] bg-[linear-gradient(180deg,rgba(24,55,120,0.92),rgba(20,48,111,0.95))] shadow-[0_22px_45px_rgba(0,0,0,0.28)] p-10 max-w-md w-full text-center">
            ...
            <a href={linkFila} className="block bg-[linear-gradient(180deg,#4b7bf0,#3d6fe0)] hover:opacity-90 text-white font-extrabold py-4 px-6 rounded-[18px] transition mb-4 shadow-[0_10px_24px_rgba(52,104,223,0.35)]">
              Ver minha fila de pedidos
            </a>
            <button onClick={() => { setSucesso(false); setItens([{ uid: uid(), itemId: null, codigo: null, descricao: '', quantidade: '1', unidade: 'un', observacao: '' }]); setObservacao(''); setJustificativaUrgencia(''); setPrazo(''); setPrioridade('normal'); setAnexos([]); }} className="text-[#b8c5eb] hover:text-white text-sm transition">
    ...
    <div className={styles.page}>
      <div className={styles.container}>
        ...
        <section className={styles.card + ' p-8 mb-6'}>
          ...
          <p className="m-0 opacity-90 text-base leading-[1.45]">Solicite materiais, ferramentas ou veículos da frota.</p>
        </section>
        ...
        <section className="grid gap-3.5 mb-5 grid-cols-1 sm:grid-cols-3">
          <TipoCard active={categoria === 'insumos'} onClick={() => handleSetCategoria('insumos')} pill="Itens" icon={<Package size={32} strokeWidth={2.2} />} titulo="Item de Estoque" descricao="Solicite itens cadastrados no estoque do almoxarifado." />
          <TipoCard active={categoria === 'ferramentas'} onClick={() => handleSetCategoria('ferramentas')} pill="Ferramentas" icon={<Wrench size={32} strokeWidth={2.2} />} titulo="Ferramenta / Equipamento" descricao="Retirada ou reserva de ferramentas e equipamentos." />
          <TipoCard active={categoria === 'frota'} onClick={() => handleSetCategoria('frota')} pill="Uso de Frota" icon={<Truck size={32} strokeWidth={2.2} />} titulo="Carro / Frota" descricao="Solicite veículo da frota para operações em obra." />
        </section>
        ...
        <form onSubmit={enviar}>
          <section className={styles.section}>
            ...
            <div className="flex flex-col gap-2.5 mb-4">
              ...
              {obra === 'Outro' && <input type="text" className={styles.input} placeholder="Digite o nome da obra" value={obraOutro} onChange={(e) => setObraOutro(e.target.value)} />}
            </div>
            <div className="grid gap-4 grid-cols-1 md:grid-cols-2 mb-4">
              <div className="flex flex-col gap-2.5">
                <label className={styles.label}>Seu nome *</label>
                <input type="text" className={styles.input} placeholder="Nome completo" value={nome} onChange={(e) => setNome(e.target.value)} required />
              </div>
              <div className="flex flex-col gap-2.5">
                <label className={styles.label}>WhatsApp *</label>
                <input type="tel" className={styles.input} placeholder="(11) 99999-9999" value={telefone} onChange={(e) => setTelefone(e.target.value)} required />
              </div>
            </div>
            <div className="flex flex-col gap-2.5">
              <label className={styles.label}>Prazo Desejado</label>
              <input type="datetime-local" step="60" min={prazoMinimo} className={styles.input} value={prazo} onChange={(e) => setPrazo(e.target.value)} />
            </div>
          </section>
          ...
          <section className={styles.section}>
            ...
            <div className="flex flex-col gap-3 mb-4">
              <label className={styles.label}>
                ...
                <span className="ml-2 normal-case font-semibold text-[#89a2e2] text-[0.8rem]">
                  ({categoria === 'insumos' ? `${insumos.length} itens cadastrados` : categoria === 'ferramentas' ? `${ferramentas.length} ferramentas cadastradas` : `${veiculos.length} veículos ativos`})
                </span>
              </label>
              <div className="flex flex-col gap-3">
                {itens.map((linha) => (
                  <ItemRow key={linha.uid} linha={linha} categoria={categoria} insumos={insumos} ferramentas={ferramentas} veiculos={veiculos} loading={loadingCatalogos} onChange={(patch) => updateLinha(linha.uid, patch)} onRemove={() => removeLinha(linha.uid)} />
                ))}
              </div>
              <div className="flex gap-2.5 mt-3">
                <button type="button" className={styles.btnSmall} onClick={addLinha}><Plus size={15} />Adicionar item</button>
              </div>
            </div>
            <div className="flex flex-col gap-2.5 mb-4">
            ...
            {prioridade === 'urgente' && (
              <div className="flex flex-col gap-2.5 mb-4">
                ...
                <textarea className={styles.input + ' min-h-[100px] resize-y'} placeholder="Explique por que é urgente" value={justificativaUrgencia} onChange={(e) => setJustificativaUrgencia(e.target.value)} />
              </div>
            )}
            <div className="flex flex-col gap-2.5">
              <label className={styles.label}>Observação</label>
              <textarea className={styles.input + ' min-h-[100px] resize-y'} placeholder="Observação (opcional)" value={observacao} onChange={(e) => setObservacao(e.target.value)} />
            </div>
          </section>

          <section className={styles.section}>
            <h4 className={styles.h4}>Anexos</h4>
            ...
            <div className="rounded-[18px] border border-dashed border-[rgba(113,154,255,0.45)] bg-[rgba(8,24,64,0.24)] px-5 py-8 text-center text-[#b8c5eb]" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); handleFiles(e.dataTransfer.files); }}>
              {anexos.length === 0 ? 'Arraste fotos ou PDFs aqui.' : `${anexos.length} arquivo${anexos.length > 1 ? 's' : ''} selecionado${anexos.length > 1 ? 's' : ''}.`}
            </div>
            {anexos.length > 0 && (
              <div className="grid gap-3 mt-4 grid-cols-2 sm:grid-cols-3">
                {anexos.map((f, i) => (
                  <div key={i} className="rounded-[14px] border border-[rgba(113,154,255,0.28)] bg-[rgba(8,24,64,0.32)] p-3 flex flex-col gap-2">
                    <div className="text-[0.8rem] text-[#b8c5eb] truncate">{f.name}</div>
          ...
          {erro && <div className="px-5 py-4 rounded-[18px] bg-[rgba(255,107,107,0.12)] border border-[rgba(255,107,107,0.35)] text-[#ff9797] mb-5">{erro}</div>}
          {sucesso && <div className="px-5 py-4 rounded-[18px] bg-[rgba(54,196,133,0.15)] border border-[rgba(54,196,133,0.4)] text-[#6be3a5] mb-5 flex items-center gap-2"><CheckCircle2 size={18} />Requisição enviada!</div>}

          <div className="flex justify-end mt-6">
            <button type="submit" disabled={salvando} className={styles.btnPrimary + ' py-4 px-8 text-[1.05rem] disabled:opacity-50'}>
