...
const DEPARTAMENTOS = ['Comercial', 'Almoxarifado', 'Engenharia', 'Administração'];
...
export function Membros() {
  ...
  async function salvarAlteracoes() {
    ...
    try {
      ...
      if (!session?.access_token) { setMensagem({ tipo: 'erro', texto: 'Sessão expirada' }); setSalvando(false); return; }
      ...
      if (!body.papel && body.departamento === undefined && !body.novaSenha) {
        setMensagem({ tipo: 'erro', texto: 'Nenhuma alteração para salvar' }); setSalvando(false); return;
      }
    ...
    } catch { setMensagem({ tipo: 'erro', texto: 'Erro de conexão' }); }
    setSalvando(false);
  }
  ...
  async function toggleAtivo(membro: Membro, novoStatus: boolean) {
    ...
    try {
      ...
      if (response.ok) setMembros(prev => prev.map(m => m.id === membro.id ? { ...m, ativo: novoStatus } : m));
    } catch { /* ignore */ }
  ...
  useEffect(() => {
    async function carregarMembros() {
      try {
        ...
        if (response.ok) {
          const data = await response.json() as Membro[];
          setMembros(data);
  ...
  return (
    <div className="space-y-6 p-4 lg:p-6 max-w-6xl mx-auto">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Todos os colaboradores da empresa — papéis, departamentos e acessos</p>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total', value: membros.length, icon: Users, bg: 'bg-blue-100', color: 'text-blue-600' },
          { label: 'Ativos', value: totalAtivos, icon: Circle, bg: 'bg-green-100', color: 'text-green-600' },
          { label: 'Online', value: totalOnline, icon: Wifi, bg: 'bg-emerald-100', color: 'text-emerald-600' },
          { label: 'Inativos', value: membros.filter(m => !m.ativo).length, icon: UserX, bg: 'bg-red-100', color: 'text-red-600' },
        ].map(({ label, value, icon: Icon, bg, color }) => (
          <div key={label} className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
            <div className="flex items-center gap-3">
              <div className={`${bg} rounded-lg p-2`}><Icon size={18} className={color} /></div>
              <div><p className="text-xl font-bold text-slate-800">{value}</p><p className="text-[10px] text-slate-500">{label}</p></div>
            </div>
          </div>
        ))}
      </div>
          ...
          {Object.entries(PAPEIS_INFO).map(([key, info]) => {
            ...
            return (
              <div key={key} className="flex items-center gap-1.5 bg-white/10 rounded-lg px-2.5 py-1.5">
                <Icone size={12} /><span className="font-medium">{info.label}</span>
              </div>
            );
          })}
        </div>
      ...
      {loading ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center text-slate-400 text-sm">Carregando membros...</div>
      ) : (
        <div className="space-y-4">
          {Object.entries(porDepto).sort(([a], [b]) => {
            const order = ['Administração', 'Comercial', 'Almoxarifado', 'Engenharia', 'Sem departamento'];
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {deptOnline > 0 && (
                      <span className="flex items-center gap-1 text-[10px] text-emerald-600 font-medium bg-emerald-50 rounded-full px-2 py-0.5">
                        <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />{deptOnline} online
                      </span>
                    )}
                  </div>
                </button>
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    {isOnline ? (
                                      <div className="flex items-center gap-1 text-[10px] text-emerald-600 font-medium">
                                        <Wifi size={9} />Online {p?.ultima_entrada ? `· ${formatarTempo(p.ultima_entrada)}` : ''}
                                      </div>
                                    ) : p?.ultimo_heartbeat ? (
                              ...
                              <td className="px-5 py-3">
                                {canManage && (
                                  <div className="flex items-center gap-1">
                                    ...
                                    {m.ativo ? (
                                      <button onClick={() => setConfirmDesativar(m)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Desativar">
                                        <UserX size={14} />
                                      </button>
                                    ) : (
                                      <button onClick={() => toggleAtivo(m, true)} className="p-1.5 rounded-lg text-slate-400 hover:text-green-600 hover:bg-green-50 transition-colors" title="Reativar">
                                        <UserCheck size={14} />
                                      </button>
                                    )}
                                  </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setConfirmDesativar(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-slate-500 mt-2"><strong>{confirmDesativar.nome}</strong> perderá acesso imediatamente.</p>
            </div>
            <div className="flex gap-3 px-6 pb-6">
              <button onClick={() => setConfirmDesativar(null)} className="flex-1 px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">Cancelar</button>
              <button onClick={() => toggleAtivo(confirmDesativar, false)} disabled={desativando} className="flex-1 px-4 py-2.5 text-sm font-medium text-white bg-red-600 hover:bg-red-700 disabled:opacity-50 rounded-xl transition-colors">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div><h3 className="font-semibold text-slate-800">{editando.nome}</h3><p className="text-xs text-slate-500">{editando.email}</p></div>
              </div>
              <button onClick={fecharModal} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400"><X size={18} /></button>
            </div>

            <div className="p-5 space-y-5">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2"><Building2 size={14} className="inline mr-1.5" />Departamento</label>
                <div className="grid grid-cols-2 gap-2">
                  {DEPARTAMENTOS.map(d => (
                    <button key={d} onClick={() => setDeptoSelecionado(d)} className={`p-2.5 rounded-xl border-2 text-xs font-medium transition-all ${deptoSelecionado === d ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 hover:border-slate-300 text-slate-600'}`}>
                      {d}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2"><KeyRound size={14} className="inline mr-1.5" />Redefinir Senha</label>
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <input type={mostrarSenha ? 'text' : 'password'} placeholder="Nova senha..." value={novaSenha} onChange={e => setNovaSenha(e.target.value)} className="w-full px-3 py-2.5 pr-20 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    <div className="absolute right-1 top-1/2 -translate-y-1/2 flex gap-0.5">
                      {novaSenha && (
                        <button onClick={copiarSenha} className="p-1.5 rounded text-slate-400 hover:text-blue-600">
                  ...
                  <button onClick={gerarSenha} className="px-3 py-2 text-xs font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-xl border border-blue-200 transition-colors">Gerar</button>
                </div>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 p-5 border-t border-slate-100">
              <button onClick={fecharModal} className="px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">Cancelar</button>
              <button onClick={salvarAlteracoes} disabled={salvando} className="px-5 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-xl transition-colors">
