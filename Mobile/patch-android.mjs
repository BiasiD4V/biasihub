...
// ── Signing config no app/build.gradle ────────────────────────────────────
// Injeta bloco signingConfigs + release buildType se ANDROID_KEYSTORE_PATH
// estiver definido (setado pelo workflow antes de rodar este script).
...
if (keystorePath && keyAlias && keyPassword && storePassword) {
  ...
  if (existsSync(buildGradlePath)) {
    ...
    if (!gradle.includes('signingConfigs')) {
      ...
      // Faz o buildType release usar signingConfig
      gradle = gradle.replace(
        /(release\s*\{(?:[^}](?!signingConfig))*?)(^\s*})/ms,
        `$1            signingConfig signingConfigs.release\n$2`
      );

      writeFileSync(buildGradlePath, gradle, 'utf8');
      console.log(`[patch-android] Signing config de release injetado em ${buildGradlePath}`);
    } else {
      console.log('[patch-android] signingConfigs ja presente, pulando injecao.');
    }
  } else {
    console.warn(`[patch-android] AVISO: build.gradle nao encontrado em ${buildGradlePath}`);
  }
} else {
  console.log('[patch-android] ANDROID_KEYSTORE_PATH nao definido — build continuara como debug.');
}
