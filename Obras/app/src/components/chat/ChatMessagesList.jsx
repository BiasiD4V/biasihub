...
function renderConteudoComLinks(text, isMine) {
  if (!text) return null;
                      ...
                      <div className={`px-3.5 py-2 text-[13px] leading-relaxed shadow-sm ${isMine ? `bg-gradient-to-br from-blue-600 to-blue-700 text-white ${consecutive ? 'rounded-2xl rounded-tr-md' : 'rounded-2xl rounded-br-md'}` : `bg-white text-slate-800 border border-slate-100 ${consecutive ? 'rounded-2xl rounded-tl-md' : 'rounded-2xl rounded-bl-md'}`}`}>
                        ...
                        {msg.arquivo_url && (
                          msg.arquivo_tipo?.startsWith('image/')
                            ...
                            ? (
                              <audio src={msg.arquivo_url} controls className="max-w-full rounded-lg" style={{ height: '36px', minWidth: '180px' }} />
                            ...
                            ? (
                              ...
                              <a href={msg.arquivo_url} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-1.5 text-xs ${isMine ? 'text-blue-200 hover:text-white' : 'text-blue-600 hover:text-blue-700'} transition-colors`}>
                                <Paperclip size={11} />
                    ...
                    {msgReacoes.length > 0 && (
                      <div className={`flex flex-wrap gap-1 mt-1 ${isMine ? 'justify-end' : 'justify-start'}`}>
                        {msgReacoes.map((r) => {
                          ...
                          return (
                            <button key={r.emoji} onClick={() => onToggleReacao(msg.id, r.emoji)} title={r.usuarios.join(', ')} className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-xs border transition-all hover:scale-105 ${jaReagi ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                              <span>{r.emoji}</span>
                    ...
                    {!isDeleted && (
                      <div className={`flex items-center gap-1 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity ${isMine ? 'justify-end mr-1' : 'ml-1'}`}>
                        <div className="relative">
                          <button onClick={() => onSetReacaoPickerAberto(reacaoPickerAberto === msg.id ? null : msg.id)} className="p-0.5 text-slate-300 hover:text-amber-500 transition-colors" title="Reagir">
                            <Smile size={10} />
                          </button>
                          {reacaoPickerAberto === msg.id && (
                            <div className={`absolute ${isMine ? 'right-0' : 'left-0'} bottom-full mb-1 flex items-center gap-0.5 bg-white border border-slate-200 rounded-full shadow-lg px-1.5 py-1 z-50`}>
                              {QUICK_REACTIONS.map((emoji) => (
                                <button key={emoji} onClick={() => onToggleReacao(msg.id, emoji)} className="w-6 h-6 flex items-center justify-center text-sm hover:bg-slate-100 rounded-full transition-colors active:scale-90">
                                  {emoji}
                                </button>
                              ))}
                            </div>
                        ...
                        <button onClick={() => onSetRespostaParaMsg(msg)} className="p-0.5 text-slate-300 hover:text-blue-500 transition-colors" title="Responder">
                          <CornerUpLeft size={10} />
                        </button>

                        {isMine && (
                          <button onClick={() => { if (confirm('Apagar esta mensagem?')) onDeletarMensagem(msg.id); }} className="p-0.5 text-slate-300 hover:text-red-500 transition-colors" title="Apagar">
                            <Trash2 size={10} />
                          </button>
                        )}
