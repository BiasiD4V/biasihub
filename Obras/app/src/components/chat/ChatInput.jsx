    ...
    <div className="border-t border-slate-100 p-3 bg-white flex-shrink-0">
      ...
      {gravando ? (
        <div className="flex items-center gap-2">
          <button onClick={onCancelarGravacao} className="p-2 text-slate-400 hover:text-slate-600 rounded-xl hover:bg-slate-100 transition-colors flex-shrink-0" title="Cancelar">
            <X size={16} />
          </button>
          ...
          <button onClick={onPararGravacao} className="p-2.5 rounded-xl bg-red-500 text-white hover:bg-red-600 transition-colors flex-shrink-0 shadow-sm shadow-red-500/25" title="Enviar audio">
            <Square size={14} fill="white" />
          </button>
        </div>
      ) : (
        <div className="flex items-center gap-2">
          <button onClick={() => fileInputRef.current?.click()} disabled={enviandoArquivo} className="p-2 text-slate-400 hover:text-slate-600 disabled:opacity-40 rounded-xl hover:bg-slate-100 transition-colors flex-shrink-0" title="Anexar arquivo">
            <Paperclip size={16} />
          </button>
                  ...
                  {EMOJI_CATEGORIES.map(cat => (
                    <div key={cat.label} className="mb-2">
                      ...
                      <div className="flex flex-wrap gap-0.5">
                        {cat.emojis.map(e => (
                          <button key={e} onClick={() => onInserirEmoji(e)} className="w-8 h-8 flex items-center justify-center text-lg hover:bg-slate-100 rounded-lg transition-colors active:scale-90">
                            {e}
                          </button>
                        ))}
                      </div>
          ...
          />
          {texto.trim() ? (
            <button onClick={onEnviarMensagem} disabled={enviandoArquivo} className="p-2.5 rounded-xl bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm shadow-blue-500/25 hover:shadow-md hover:shadow-blue-500/30 active:scale-95">
              {enviandoArquivo
            ...
            <button onClick={onIniciarGravacao} disabled={enviandoArquivo} className="p-2.5 rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-slate-700 disabled:opacity-40 transition-all active:scale-95" title="Gravar audio">
              <Mic size={14} />
            </button>
          )}
        </div>
