...
export default function BoasVindas() {
  ...
  return (
    <div className="obras-boas-vindas p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {ATALHOS.map(grupo => (
          <div key={grupo.grupo} className="rounded-xl overflow-hidden" style={{ backgroundColor: '#112b61', border: '1px solid #3f63ab' }}>
            {/* Cabeçalho do grupo */}
            <div className="px-4 py-2.5 flex items-center gap-2" style={{ backgroundColor: 'rgba(11, 29, 69, 0.92)', borderBottom: '1px solid #3f63ab' }}>
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: grupo.cor }} />
              <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#dce8ff' }}>{grupo.grupo}</span>
            </div>
            {/* Itens */}
            <div className="divide-y" style={{ borderColor: '#365894' }}>
              {grupo.itens.map(item => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[#173872] group"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold truncate" style={{ color: '#f2f7ff' }}>{item.label}</p>
                      <p className="text-[11px] truncate" style={{ color: '#c7d8ff' }}>{item.desc}</p>
                    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* Avisos */}
        <div className="rounded-xl overflow-hidden" style={{ backgroundColor: '#112b61', border: '1px solid #3f63ab' }}>
          <div className="px-5 py-3.5 flex items-center gap-2" style={{ borderBottom: '1px solid #3f63ab' }}>
            <Megaphone size={15} style={{ color: '#8fb3ff' }} />
            <span className="text-sm font-bold" style={{ color: '#f2f7ff', fontFamily: 'Montserrat, sans-serif' }}>Avisos & Novidades</span>
          </div>
          <div className="divide-y" style={{ borderColor: '#365894' }}>
            {AVISOS.map((a, i) => {
              const Icon = a.icone
              return (
                <div key={i} className="flex items-start gap-3 px-5 py-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm mt-0.5" style={{ color: '#e7f0ff' }}>{a.texto}</p>
                  </div>
                </div>
              )
            })}
        ...
        <div className="rounded-xl overflow-hidden" style={{ backgroundColor: '#112b61', border: '1px solid #3f63ab' }}>
          <div className="px-5 py-3.5 flex items-center gap-2" style={{ borderBottom: '1px solid #3f63ab' }}>
            <Clock size={15} style={{ color: '#8fb3ff' }} />
            <span className="text-sm font-bold" style={{ color: '#f2f7ff', fontFamily: 'Montserrat, sans-serif' }}>Histórico de Atualizações</span>
          </div>
          <div className="divide-y" style={{ borderColor: '#365894' }}>
            {ATUALIZACOES.map((a, i) => (
              <div key={i} className="flex items-start gap-3 px-5 py-4">
                <span className="text-[11px] font-mono font-semibold flex-shrink-0 mt-0.5 px-2 py-0.5 rounded"
                  style={{ backgroundColor: '#173872', color: '#cfe0ff', border: '1px solid #4d72ba' }}>
                  {a.data}
                </span>
                <p className="text-sm" style={{ color: '#e7f0ff' }}>{a.texto}</p>
              </div>
            ))}
          </div>
          <div className="px-5 py-3 text-center text-[11px]" style={{ color: '#c7d8ff', borderTop: '1px solid #3f63ab' }}>
