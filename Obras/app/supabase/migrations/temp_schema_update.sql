...
CREATE POLICY "eap_supervisor_select" ON planejamento_eap
  ...
  USING (
    ...
    AND EXISTS (
      ...
      WHERE op.id = planejamento_eap.planejamento_id AND uo.usuario_id = auth.uid()
    )
  );
...
CREATE POLICY "atividades_supervisor_select" ON planejamento_atividades
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1 FROM obra_planejamentos op
      JOIN usuario_obra uo ON uo.obra_id = op.obra_id
      WHERE op.id = planejamento_atividades.planejamento_id AND uo.usuario_id = auth.uid()
    )
  );
...
CREATE POLICY "avancos_supervisor_select" ON avancos_fisicos
  ...
  USING (
    ...
    AND EXISTS (
      ...
      JOIN usuario_obra uo ON uo.obra_id = op.obra_id
      WHERE pa.id = avancos_fisicos.atividade_id AND uo.usuario_id = auth.uid()
    )
  );
...
CREATE POLICY "avancos_supervisor_update" ON avancos_fisicos
  ...
  WITH CHECK (EXISTS (SELECT 1 FROM perfis WHERE id = auth.uid() AND perfil = 'supervisor'));
...
CREATE POLICY "reprog_supervisor_select" ON reprogramacoes
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1 FROM obra_planejamentos op
      JOIN usuario_obra uo ON uo.obra_id = op.obra_id
      WHERE op.id = reprogramacoes.planejamento_id AND uo.usuario_id = auth.uid()
    )
  );
...
CREATE POLICY "evm_supervisor_select" ON evm_snapshots
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1 FROM obra_planejamentos op
      JOIN usuario_obra uo ON uo.obra_id = op.obra_id
      WHERE op.id = evm_snapshots.planejamento_id AND uo.usuario_id = auth.uid()
    )
  );
